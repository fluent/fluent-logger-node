  ...
  this._server = net.createServer((socket) => {
    ...
    socket.pipe(stream).on('data', (m) => {
      if (this._state === 'pingpong') {
        let authResult = this._checkPing();
...
MockFluentdServer.prototype._checkPing = function() {
  // this._options.checkPing() should return { succeeded: true, reason: "why", salt: "salt" }
  return this._options.checkPing();
};
