...
FluentSender.prototype.toStream = function(label){
  var self = this;
  var writable = new stream.Writable();
  var dataString = '';
  writable._write = function(chunk, encoding, callback){
    var dataArray = chunk.toString('UTF-8').split(/\n/);
    function next(cb){
      if (dataArray.length) dataString += dataArray.shift();
      if (!dataArray.length) return process.nextTick(cb);
      self.emit(label, dataString, function(){
        dataString = '';
        next(cb);
      });
    }
