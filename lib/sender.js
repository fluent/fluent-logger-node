        ...
        var intervalId = setInterval(function() {
          if (self._data) {
            ...
            if (response.ack !== item.options.chunk) {
              if (self._eventEmitter.listeners('error').length > 0) {
                self._eventEmitter.emit('error', 'ack in response and chunk id in sent data are different');
              }
            }
          }
        }, 100);
        setTimeout(function() {
          if (self._eventEmitter.listeners('response-timeout').length > 0) {
            self._eventEmitter.emit('response-timeout', 'ack response timeout');
          }
          clearInterval(intervalId);
