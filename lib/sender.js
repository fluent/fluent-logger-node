...
function FluentSender(tag_prefix, options){
  options = options || {};
  this._forwardMode = options.forwardMode || 'Message'; // Message or PackedForward
    ...
    if (this._socket.writable) {
      if (this._forwardMode === 'Message') {
        ...
        if (this._sendQueueSize >= this._sendQueueSizeLimit) {
          ...
          this._flushSendQueueTimeoutId && clearTimeout(this._flushSendQueueTimeoutId);
          this._flushSendQueueTimeoutId = setTimeout(this._doFlushSendQueue, this._flushInterval);
        }
      }
...
FluentSender.prototype._doFlushSendQueue = function(timeoutId) {
  if (this._forwardMode === 'Message') {
    ...
    let packet = msgpack.encode([tag, eventEntryData.eventEntry, options], { codec: codec });
