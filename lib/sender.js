...
FluentSender.prototype._doFlushSendQueue = function(timeoutId) {
  this._flushPendingPackets(timeoutId);
  if (this._eventMode === 'Message') {
...
FluentSender.prototype._doWrite = function(packet, options, timeoutId, callbacks) {
  if (!this._socket) {
    ...
    this._pendingPackets.push([packet, options, callbacks]);
    this._handleEvent('error', error);
    return;
  }
...
FluentSender.prototype._flushPendingPackets = function _flushPendingPackets(timeoutId) {
  let pendingPacket = this._pendingPackets.shift();
  while (pendingPacket !== undefined) {
    let packet = pendingPacket[0];
    let options = pendingPacket[1];
    let callbacks = pendingPacket[2];
    this._doWrite(packet, options, timeoutId, callbacks);
    pendingPacket = this._pendingPackets.shift();
  }
};
