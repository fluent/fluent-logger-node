...
FluentSender.prototype.emit = function(label, data, timestamp, callback){
  ...
  this._connect(this._flushSendQueue.bind(this));
};
...
FluentSender.prototype._flushSendQueue = function(){
  ...
  if( this._socket && this._socket.writable ){
    var item = this._sendQueue[0];
    if( item === undefined ){
      // nothing written;
    }else{
      this._sendQueue.shift();
      this._socket.write(new Buffer(item.packet), function(){
        // TODO: how should we recover if dequeued items are not sent.
        item.callback && item.callback();
      });
      process.nextTick(this._flushSendQueue.bind(this));
    }
  }
};
